...
BD_EP3 <- BD_General %>% 
  ...
  rename(Edad_r= Edad_Real, Diametro_Fibra = DM_FLc2022)  
BD_EP3
```
...
```{r}
std.error(BD_EP)
>>>>>>> 219a04739f864197ea61b5db0e36707a79a65288
```
...
```{r}
BD_EP2 <- select(BD_General, "Condicion_Corporal", "DM_FLc2022", "Edad")
BD_EP2
```
```{r}
ggplot(BD_General, aes(x=Edad_r, y=DM_FLc2022)) +
  geom_point(aes(color = Edad_r))
...
```{r}
ggplot(BD_General, aes(x=Edad_r, y=DM_FLc2022)) +
  geom_violin(aes(color = Edad_r, fill = Edad_r))
```
#
